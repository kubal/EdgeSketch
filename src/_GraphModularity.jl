...
# Compute modularity gain when adding node v to community (slow, for reference)
...
# Compute modularity gain when adding node v to community (fast, used by Louvain)
...
# Aggregate communities into super-nodes for hierarchical Louvain
...
# Map super-node communities back to original node communities
...
# Louvain method phase 1: iteratively move nodes to maximize modularity
function louvain_method(adj_list::Vector{Vector{Tuple{Int, Float64}}})
...
# Louvain method phase 2: aggregate and re-run on super-graph
...
# Group nodes by their community, e.g. [1,2,1,2] -> [{1,3}, {2,4}]
function blocks_to_communities(blocks::Vector{Int})::Vector{Set{Int}}
